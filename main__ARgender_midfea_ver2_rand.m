...
%}
%load('AR_300dim');
load('midFeaActv1_300_3DPoolRandProj');
...
hiddenSize = classNum * 10;

MAXITER = 50;

% parameters for initializing W and b
iniMax = 10; % for updating Hc
...
%r = sqrt(16) / sqrt(hiddenSize+size(data,2)+1); 
H = rand(hiddenSize, size(data,2)) ;
...
while iterCount < MAXITER
    ...
    [W, b, cost] = updateWb(W, b, data, H, 3, iniMax*100);
  %  figure('name', ['convergence check at iteration-' num2str(iterCount)]);
   ...
   % testSetH = sigmoid(W*testSetActv1+repmat(b, 1, size(testSetActv1,2) ));
    ...
    trainSetH = (D'*D)\D'*data;
    testSetH = (D'*D)\D'*testSetActv1;
    
    %trainSetH = omp(D'*data, D'*D, nonZeroNum);
    %testSetH = omp(D'*testSetActv1, D'*D, nonZeroNum);
    
    trainSetH = trainSetH ./ repmat( sqrt(sum(trainSetH.^2,1)), size(trainSetH,1), 1);
    testSetH = testSetH ./ repmat( sqrt(sum(testSetH.^2,1)), size(testSetH,1), 1);
